...
public class SIPCalculatorPageObjects {
	...
	public void enterOnInvestAmount(ExtentTest test, String data) {
		try {
			String message = "Entered value  on Investment Amount";
			webDriverHelper.clearElement(sipCalculatorPageLocators.getInvestAmt());
			webDriverHelper.sendKeys(sipCalculatorPageLocators.getInvestAmt(), ExcelReader.readDatafromExcel(excelFilePath, sheetName, Integer.parseInt(data), 6));
			test.pass(message);
			LoggerHandler.logInfo(message);
		} catch (Exception e) {
	...
	public void enterOnInvestYears(ExtentTest test, String data) {
		try {
			String message = "Entered value  on Investment Years";
			webDriverHelper.clearElement(sipCalculatorPageLocators.getInvestYears());
			webDriverHelper.sendKeys(sipCalculatorPageLocators.getInvestYears(), ExcelReader.readDatafromExcel(excelFilePath, sheetName, Integer.parseInt(data), 6));
			test.pass(message);
			LoggerHandler.logInfo(message);
		} catch (Exception e) {
	...
	public void enterOnExpectedReturn(ExtentTest test, String data) {
		try {
			webDriverHelper.clearElement(sipCalculatorPageLocators.getExpectedReturn());
			webDriverHelper.sendKeys(sipCalculatorPageLocators.getExpectedReturn(), ExcelReader.readDatafromExcel(excelFilePath, sheetName, Integer.parseInt(data), 6));
